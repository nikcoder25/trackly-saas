...
function getGlobalCacheKey(platform, model, query, city) {
  const cityPart = city ? `::${city.trim().toLowerCase()}` : '';
  return `global::${platform}::${model}::${query.trim().toLowerCase()}${cityPart}`;
}
...
async function getCachedResponseWithDb(platform, model, query, brandId, city) {
  ...
  if (globalDbResult) {
    _setMemoryCache(globalKey, globalDbResult);
...
function setCachedResponse(platform, model, query, result, brandId, city) {
  ...
  const ttl = getCacheTTL(platform, model);
  setDbCachedResponse(key, platform, model, query.trim().toLowerCase(), brandId, city, result, isSearch, ttl).catch(() => {});
  // Also store global cache in DB for cross-brand sharing
  setDbCachedResponse(globalKey, platform, model, query.trim().toLowerCase(), null, city, result, isSearch, ttl).catch(() => {});
}
...
async function fetchJSONOnce(url, options) {
  return new Promise((resolve, reject) => {
    ...
    const reqOptions = {
      ...
      timeout: 15000, // 15s idle timeout (reduced for speed) — AI responses arrive within 10-20s
    };
    if (body) reqOptions.headers['Content-Length'] = Buffer.byteLength(body);

    // Hard deadline: abort if total request exceeds 25s (was 90s).
    // With max_tokens=600, responses are short and arrive fast.
    let settled = false;
    const hardTimeout = setTimeout(() => {
      if (!settled) { settled = true; req.destroy(); reject(new Error('Request timeout after 25s to ' + new URL(url).hostname)); }
    }, 25000);
...
module.exports = { queryAI, fetchJSON, SYSTEM_PROMPT, PLATFORM_MODELS, getDefaultModel, PLATFORM_RATE_LIMITS, resetBatchCount, MODEL_PRICING, estimateCost, runClaudeBatch };
