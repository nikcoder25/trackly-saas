...
async function queryAI(query, platform, brand, keys, modelPrefs, logContext) {
  ...
  try {
    ...
    if (ctx.logFn) {
      ctx.logFn({ userId: ctx.userId, brandId: ctx.brandId, platform, query: rawQuery, status: 'error', error: e.message, keyHint, model: modelUsed, responseMs: Date.now() - startMs, tokensIn: 0, tokensOut: 0, cost: null });
    }
    throw e;
  }
  ...
  // Log successful call
  if (ctx.logFn) {
    ctx.logFn({ userId: ctx.userId, brandId: ctx.brandId, platform, query: rawQuery, status: 'ok', error: null, keyHint, model: modelUsed, responseMs: Date.now() - startMs, tokensIn, tokensOut, cost });
  }
