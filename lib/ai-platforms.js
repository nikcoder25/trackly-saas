...
async function callGemini(query, apiKey, model) {
  ...
  for (let attempt = 0; attempt <= MAX_GEMINI_RETRIES; attempt++) {
    ...
    if (d.error) {
      ...
      const isTransient = msg.includes('high demand') || msg.includes('try again later') || msg.includes('overloaded') || msg.includes('resource exhausted');
      if (isTransient && attempt < MAX_GEMINI_RETRIES) {
        const delay = (2000 * Math.pow(2, attempt)) + Math.floor(Math.random() * 1000);
        logger.warn(`Gemini transient error (attempt ${attempt + 1}/${MAX_GEMINI_RETRIES}): ${msg.slice(0, 100)}. Retrying in ${delay}ms...`);
        await sleep(delay);
        continue;
      }
...
async function queryAI(query, platform, brand, keys, modelPrefs, logContext) {
  ...
  // Circuit breaker: try keys in order, skip any with too many auth failures
  let apiKey = null;
  for (const candidateKey of keysArray) {
    if (!circuitBreakerCheck(candidateKey)) {
      apiKey = candidateKey;
      break;
    }
  }
  // If all keys are circuit-broken, try the first one anyway (it may have recovered)
  if (!apiKey) apiKey = rotateKey(keysArray, platform);
  ...
  try {
    ...
    error = e.message;
    // Track auth failures for circuit breaker (401/403 errors)
    const errMsg = (e.message || '').toLowerCase();
    if (errMsg.includes('auth error') || errMsg.includes('401') || errMsg.includes('403') || errMsg.includes('invalid api key') || errMsg.includes('invalid x-api-key') || errMsg.includes('permission')) {
...
module.exports = { queryAI, fetchJSON, PLATFORM_MODELS, getDefaultModel, resetBatchCount, MODEL_PRICING, estimateCost, runClaudeBatch, runOpenAIBatch, circuitBreakerCheck, recordApiKeyFailure, resetApiKeyFailures };
