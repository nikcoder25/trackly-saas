...
async function callOpenAI(query, apiKey, model, brand) {
  // Use search-preview model to get web-grounded results (matches real ChatGPT behavior)
  const useModel = 'gpt-4o-search-preview';
  const payload = {
  ...
  // Add user location for better local search results
  if (brand && brand.city) {
...
async function queryAI(query, platform, brand, keys) {
  ...
  if (platform === 'ChatGPT' && keys.openai)    return await callOpenAI(rawQuery, keys.openai, 'gpt-4o', brand);
