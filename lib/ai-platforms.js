...
const CACHE_TTL_STATIC_MS = 24 * 60 * 60 * 1000; // 24h for non-search
const CACHE_TTL_SEARCH_MS = 1 * 60 * 60 * 1000;  // 1h for search models
const CACHE_MAX_ENTRIES = 10000;

function getCacheKey(platform, model, query, brandId) {
  const prefix = brandId ? `${brandId}::` : '';
  return `${prefix}${platform}::${model}::${query.trim().toLowerCase()}`;
}
...
function getCachedResponse(platform, model, query, brandId) {
  const key = getCacheKey(platform, model, query, brandId);
...
function setCachedResponse(platform, model, query, result, brandId) {
...
// All platforms are now cacheable — search models get shorter TTL (1h)
...
async function queryAI(query, platform, brand, keys, modelPrefs, logContext) {
  ...
  // Check response cache (isolated per brand)
  if (isCacheable(platform, resolvedModel)) {
    const cached = getCachedResponse(platform, resolvedModel, rawQuery, ctx.brandId);
    if (cached) {
      if (ctx.logFn) {
  ...
  // Store in cache (isolated per brand)
  if (result && isCacheable(platform, resolvedModel)) {
    setCachedResponse(platform, resolvedModel, rawQuery, result, ctx.brandId);
  }
