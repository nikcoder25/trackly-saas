...
const PLATFORM_RATE_LIMITS = {
  ChatGPT:      { minDelayMs: 3000, rpm: 20 },
  Claude:       { minDelayMs: 2000, rpm: 50 },
  Gemini:       { minDelayMs: 6000, rpm: 10 },
  'Google AIO': { minDelayMs: 6000, rpm: 10 },
  Grok:         { minDelayMs: 2000, rpm: 60 },
  Perplexity:   { minDelayMs: 2000, rpm: 50 },
  DeepSeek:     { minDelayMs: 1500, rpm: 60 },
  Mistral:      { minDelayMs: 3000, rpm: 20 }
};
...
async function rateLimitWait(platform, apiKey) {
  ...
  if (!limits) return;
  // Track per-key to allow parallel usage across multiple keys
  const trackKey = apiKey ? (platform + ':' + apiKey.slice(-8)) : platform;
  const now = Date.now();
...
async function callGrok(query, apiKey, model) {
  ...
  // grok-3-mini does not support web_search in the Responses API — use Chat Completions API instead
  const modelsWithoutWebSearch = ['grok-3-mini', 'grok-3-mini-fast'];
  if (modelsWithoutWebSearch.includes(grokModel)) {
    const body = JSON.stringify({
      model: grokModel,
      messages: [
        { role: 'system', content: SYSTEM_PROMPT },
    ...
    return { text: text.trim(), simulated: false, citations: [], model: d.model || grokModel };
  }

  // Use Responses API with web_search for models that support it
  const body = JSON.stringify({
    model: grokModel,
...
async function queryAI(query, platform, brand, keys, modelPrefs) {
  ...
  if (platform === 'ChatGPT')    return await callOpenAI(rawQuery, apiKey, prefs.ChatGPT, brand);
  if (platform === 'Perplexity') return await callPerplexity(rawQuery, apiKey, prefs.Perplexity);
  if (platform === 'Gemini')     return await callGemini(rawQuery, apiKey, prefs.Gemini);
  if (platform === 'Grok')       return await callGrok(rawQuery, apiKey, prefs.Grok);
  if (platform === 'Claude')     return await callClaude(rawQuery, apiKey, prefs.Claude);
  if (platform === 'Google AIO') return await callGeminiWithSearch(rawQuery, apiKey, prefs['Google AIO']);
  if (platform === 'DeepSeek')   return await callDeepSeek(rawQuery, apiKey, prefs.DeepSeek);
  if (platform === 'Mistral')    return await callMistral(rawQuery, apiKey, prefs.Mistral);
