...
// System prompt — compressed for token savings (~40% fewer tokens than original)
// Original: 42 tokens → Optimized: ~25 tokens. Saves ~17 tokens × every API call.
const SYSTEM_PROMPT = 'Recommendation ssistant. Name specific businesses/brands with full names. List 5-10 with brief descriptions. Max 200 words.';

// Max output tokens — system prompt asks for max 200 words (~250 tokens).
// 400 tokens gives enough headroom. Output tokens are expensive (e.g. Claude $4/1M).
const MAX_OUTPUT_TOKENS = 300;

// ── Response cache (all models) ─────────────────────────────────
// Non-search models: 48h TTL (static knowledge, no web grounding)
// Search models: 24h TTL (web-grounded but saves cost on repeated queries)
const _responseCache = new Map();
const CACHE_TTL_STATIC_MS = 7 * 24 * 60 * 60 * 1000; // 7 days for non-search (static knowledge changes slowly — saves ~60% on Gemini/DeepSeek repeat queries)
const CACHE_TTL_SEARCH_MS = 24 * 60 * 60 * 1000;    // 24h for search models (web-grounded, balances freshness vs cost)
...
function _isSearchModel(platform, model) {
  ...
  return false; // Gemini (plain), DeepSeek — no web search
}
...
const PLATFORM_RATE_LIMITS = {
  ...
  'Google AIO': { minDelayMs: 150,  rpm: 1000 }, // Same provider as Gemini (faster)
  Grok:         { minDelayMs: 250,  rpm: 120 },  // xAI paid: ~60-120 RPM (faster)
  Perplexity:   { minDelayMs: 300,  rpm: 100 },  // Paid: ~100 RPM (faster)
  DeepSeek:     { minDelayMs: 150,  rpm: 150 },  // Very generous limits (faster)
};
...
async function fetchJSON(url, options) {
  const MAX_RETRIES = 2;              // Was 3 — fail faster, let caller handle
  const NETWORK_BASE_DELAY = 500;     // 0.5s, 1s for network errors (faster)
  const RATE_LIMIT_BASE_DELAY = 3000; // 3s, 6s for rate limits (faster)
  let lastError;
...
function matchesPlatformHost(host, platform) {
  const map = { ChatGPT:'openai', Claude:'anthropic', Gemini:'googleapis', 'Google AIO':'googleapis', Grok:'x.ai', Perplexity:'perplexity', DeepSeek:'deepseek' };
...
async function createClaudeBatch(queries, apiKey, model) {
  ...
  const d = await fetchJSON('https://api.anthropic.com/v1/messages/batches', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'x-api-key': cleanKey,
...
async function pollClaudeBatch(batchId, apiKey, maxWaitMs) {
  const cleanKey = (apiKey || '').trim();
  const maxWait = maxWaitMs || 5 * 60 * 1000; // 5 min default
  const startTime = Date.now();
  const pollInterval = 10000; // 10s between polls

  while (Date.now() - startTime < maxWait) {
    const d = await fetchJSON(`https://api.anthropic.com/v1/messages/batches/${batchId}`, {
      method: 'GET',
      headers: {
        'x-api-key': cleanKey,
        'anthropic-version': '2023-06-01'
...
async function getClaudeBatchResults(batchId, apiKey) {
  ...
  return new Promise((resolve, reject) => {
    const url = `https://api.anthropic.com/v1/messages/batches/${batchId}/results`;
...
async function createOpenAIBatch(queries, apiKey, model, brand) {
  ...
  const uploadResult = await fetchJSON('https://api.openai.com/v1/files', {
    method: 'POST',
    headers: {
  ...
  // Create batch
  const batchResult = await fetchJSON('https://api.openai.com/v1/batches', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': 'Bearer ' + cleanKey
...
async function pollOpenAIBatch(batchId, apiKey, maxWaitMs) {
  const cleanKey = (apiKey || '').trim();
  const maxWait = maxWaitMs || 5 * 60 * 1000;
  const startTime = Date.now();
  const pollInterval = 10000;

  while (Date.now() - startTime < maxWait) {
    const d = await fetchJSON(`https://api.openai.com/v1/batches/${batchId}`, {
      method: 'GET',
...
async function getOpenAIBatchResults(outputFileId, apiKey) {
  const cleanKey = (apiKey || '').trim();
  return new Promise((resolve, reject) => {
    const url = `https://api.openai.com/v1/files/${outputFileId}/content`;
    const req = https.request(url, {
      method: 'GET',
      headers: { 'Authorization': 'Bearer ' + cleanKey },
      timeout: 30000
...
module.exports = { queryAI, fetchJSON, SYSTEM_PROMPT, PLATFORM_MODELS, getDefaultModel, PLATFORM_RATE_LIMITS, resetBatchCount, MODEL_PRICING, estimateCost, runClaudeBatch, runOpenAIBatch, normalizeQuery };
