...
function detectDiagnosticEvents(current, baseline) {
  ...
  // Visibility drop
  if (baseline.mentionRate > 0 && current.mentionRate < baseline.mentionRate * THRESHOLDS.visibilityDropMultiplier) {
    events.push({
  ...
  // Visibility gain
  if (current.mentionRate > baseline.mentionRate * THRESHOLDS.visibilityGainMultiplier && current.mentionRate - baseline.mentionRate > THRESHOLDS.visibilityGainAbsoluteMin) {
    events.push({
