...
async function sendReportEmail(to, brandName, report) {
  ...
  const html = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p style="color: #6b7280;">Period: ${escHtml(report.period?.from || 'N/A')} to ${escHtml(report.period?.to || 'N/A')}</p>
...
async function sendAlertEmail(to, alertName, message) {
  ...
  const html = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p style="color:#999;font-size:12px;">You're receiving this because you configured alert rules in Trackly. Manage alerts in your dashboard settings.</p>
    </div>
  `;
