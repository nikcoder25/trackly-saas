...
async function sendEmail(to, subject, html) {
  if (!isEmailConfigured()) {
    const missing = [!EMAIL_FROM && 'EMAIL_FROM', !EMAIL_API_KEY && 'EMAIL_API_KEY', !EMAIL_API_URL && 'EMAIL_API_URL', !APP_URL && 'APP_URL'].filter(Boolean);
...
async function sendReportEmail(to, brandName, report) {
  ...
  const html = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p style="color:${EMAIL_COLORS.light};font-size:12px;margin-top:16px;">You're receiving this because you enabled scheduled reports. Manage settings in your Trackly dashboard.</p>
    </div>
  `;
  return sendEmail(to, `AI Visibility Report: ${escHtml(brandName)} - Trackly`, html);
}
...
async function sendAlertEmail(to, alertName, message) {
  ...
  const html = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p style="color:${EMAIL_COLORS.light};font-size:12px;">You're receiving this because you configured alert rules in Trackly. Manage alerts in your dashboard settings.</p>
    </div>
  `;
  return sendEmail(to, `Alert: ${escHtml(alertName)} - Trackly`, html);
}
