...
async function generateRecommendations(brandId, analytics) {
  ...
  try {
    ...
    if (analytics.overallMentionRate !== undefined && analytics.overallMentionRate < THRESHOLDS.lowVisibility) {
    ...
    if (analytics.topCompetitors && analytics.topCompetitors.length > 0) {
      for (const comp of analytics.topCompetitors) {
        if (comp.mentionRate > (analytics.overallMentionRate || 0) * THRESHOLDS.competitorMultiplier && comp.mentionRate > THRESHOLDS.competitorDomination) {
    ...
    if (analytics.platformBreakdown) {
      ...
      if (platforms.length >= 2) {
        ...
        for (const [platform, data] of platforms) {
          ...
          if (rate < avgRate * THRESHOLDS.platformGapMultiplier && avgRate > THRESHOLDS.missingCitationMinRate) {
...
module.exports = { generateRecommendations, getPlaybook, getAllPlaybooks };
