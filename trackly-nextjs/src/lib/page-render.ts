/**
 * Shared page-render / unblocker cascade.
 *
 * Some pages cannot be read with a plain HTTP GET: they render their body
 * with JavaScript, or they sit behind Cloudflare and answer a datacentre IP
 * with a challenge. This module is the escape hatch - a chain of backends
 * that will fetch such a page as a real browser would, tried in order and
 * degrading to null when none is configured.
 *
 * Extracted from nap-audit-run.ts, which was its only caller. It is shared
 * now because competitor content analysis has exactly the same problem and
 * a much worse failure mode: a NAP check that cannot read a page reports
 * "blocked" and moves on, whereas a content comparison that silently reads
 * an empty JS shell concludes the competitor has no content and invents a
 * gap that is not there. Anything measuring what is ON a third-party page
 * must render it or know that it could not.
 *
 * Cost note: the paid backends bill per successful request (ScraperAPI
 * ~10-30 credits, Zyte and Bright Data pay-per-success), so callers should
 * treat a render as expensive and cap how many they do per run.
 *
 * Every backend is optional. With none configured `renderServiceEnabled()`
 * is false and `renderHtml()` returns null, and callers must fall back to
 * their ordinary fetch path rather than treating that as an empty page.
 */
...
/**
 * True when a PAID backend is configured. Callers distinguish this from
 * renderServiceEnabled() because the free fallbacks read an archived or
 * proxied copy: worth having when the live fetch failed outright, not worth
 * having when it succeeded and we only wanted a better read of it.
 */
export function paidRenderEnabled(): boolean {
  return PAID_UNBLOCKER_ENABLED;
}
 ...
 * Layer 1/2 result.
 */
export async function renderHtml(url: string, includeArchive: boolean): Promise<RenderResult | null> {
  const live =
...
// ── robots.txt gate (third-party crawls only) ────────────────────────────────
//
// The engine's own crawler does not consult robots.txt, and for its usual job
// that is defensible: it reads the customer's own site, at the customer's
// request, on pages they asked us to fix. Competitor analysis is a different
// posture. We are fetching someone else's pages, on a schedule, for our
// customer's commercial benefit, and the only signal that site has to express
// how it wants automated clients to behave is robots.txt. So we read it.
//
// This is a deliberately small implementation of the parts of the standard
// that matter here: User-agent grouping, Disallow, Allow, and longest-match
// precedence. It does not implement Crawl-delay or Sitemap. A fetch failure
// is treated as "allowed", matching the standard's guidance that an
// unreachable robots.txt does not imply a blanket disallow.

/** How long a parsed robots.txt stays cached, in ms. */
...
/**
 * Parse robots.txt into the rule set applying to `ua`.
 *
 * Group selection follows the standard: a group naming our user-agent wins
 * outright, and the `*` group applies only when no specific group matched.
 */
export function parseRobots(text: string, ua = 'livesov'): RobotsRules {
  const specific: RobotsRules = { allow: [], disallow: [] };
  const wildcard: RobotsRules = { allow: [], disallow: [] };
  // Which groups the current block applies to. A run of consecutive
  // User-agent lines shares one rule block.
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    ...
    if (field === 'user-agent') {
      if (!lastLineWasAgent) { inSpecific = false; inWildcard = false; }
      const agent = value.toLowerCase();
      if (agent === '*') inWildcard = true;
      else if (agent && ua.toLowerCase().includes(agent)) inSpecific = true;
...
/** Length of the longest matching rule, or -1 when none match. */
function longestMatch(rules: string[], path: string): number {
  let best = -1;
  for (const rule of rules) {
    if (!rule) continue;
    // Only `*` and a trailing `$` are wildcards in the standard.
    const pattern = rule
      .replace(/[.+?^${}()|[\]\\]/g, '\\$&')
      .replace(/\*/g, '.*');
    const anchored = pattern.endsWith('\\$') ? `^${pattern.slice(0, -2)}$` : `^${pattern}`;
    try {
      if (new RegExp(anchored).test(path)) best = Math.max(best, rule.length);
    } catch { /* skip an unparseable rule */ }
  }
  return best;
}

/** True when robots.txt permits fetching `url`. Fail-open on any error. */
export async function robotsAllows(url: string, ua = 'livesov'): Promise<boolean> {
  let target: URL;
  ...
  if (!entry || Date.now() - entry.at > ROBOTS_TTL_MS) {
    ...
    try {
      ...
      // 4xx (including 404) means no restrictions published. A 5xx arguably
      // means "unknown", but treating a flaky server as a blanket disallow
      // would make the feature randomly unavailable, so we allow and cache
      // briefly.
      if (res.ok) rules = parseRobots(await res.text(), ua);
    } catch (e) {
      logger.warn('page_render.robots_fetch_failed', { origin, err: (e as Error).message });
    } finally {
      clearTimeout(timer);
    }
  ...
  const disallowed = longestMatch(entry.rules.disallow, path);
  if (disallowed < 0) return true;
  // Allow wins ties, per the standard.
  return longestMatch(entry.rules.allow, path) >= disallowed;
}

/** Test seam: drop the cached robots.txt rules. */
export function __resetRobotsCache(): void { robotsCache.clear(); }
