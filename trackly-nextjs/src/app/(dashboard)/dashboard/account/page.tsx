...
export default function AccountPage() {
  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="bg-[var(--bg2)] border border-[var(--border)] rounded-xl p-6 mb-6">
        ...
        <div className="space-y-3 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span className="text-[var(--text)] capitalize">{user?.plan}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-[var(--text-muted)]">2FA</span>
            <span className={user?.totpEnabled ? 'text-[var(--success)]' : 'text-[var(--text-muted)]'}>
      ...
      {/* Username */}
      <div className="bg-[var(--bg2)] border border-[var(--border)] rounded-xl p-6 mb-6">
        <h2 className="text-lg font-semibold text-[var(--text)] mb-4">Username</h2>
        <form onSubmit={handleSaveUsername} className="flex gap-3">
          <input
            type="text"
            value={username}
            onChange={(e) => setUsername(e.target.value)}
            className="flex-1 bg-[var(--bg)] border border-[var(--border)] rounded-lg px-4 py-2 text-[var(--text)] text-sm focus:outline-none focus:border-[var(--primary)]"
            placeholder="your-username"
          />
          <button type="submit" disabled={saving}
            className="bg-[var(--primary)] hover:bg-[var(--primary-hover)] text-[var(--text)] px-4 py-2 rounded-lg text-sm transition disabled:opacity-50">
            {saving ? 'Saving...' : 'Save'}
          </button>
        </form>
