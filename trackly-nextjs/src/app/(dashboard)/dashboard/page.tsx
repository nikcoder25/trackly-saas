  ...
  return (
    <div>
      ...
      {show('queries')&&<div className="ov-card" style={{marginBottom:0}}><div className="ov-card-head"><div className="ov-card-title">Tracked Queries</div><div className="ov-card-sub">{queries.length} / {planLimit>1000?'∞':planLimit} prompts</div></div>
        <div className="ov-query-tags" style={{display:'flex',flexWrap:'wrap',gap:6,marginBottom:12}}>{queries.map((q,i)=>{const isSelected=selectedQueries.has(i);return <span key={i} title={q} onClick={selectMode?()=>{setSelectedQueries(prev=>{const next=new Set(prev);if(next.has(i))next.delete(i);else next.add(i);return next;});}:undefined} style={{display:'inline-flex',alignItems:'center',gap:6,background:selectMode&&isSelected?'var(--primary-light, rgba(99,102,241,.12))':'var(--bg3)',border:selectMode&&isSelected?'1px solid var(--primary)':'1px solid var(--border)',color:'var(--text)',fontSize:12,padding:'6px 12px',borderRadius:100,cursor:selectMode?'pointer':'default',userSelect:'none'}}>{selectMode&&<span style={{fontSize:13,lineHeight:1,color:isSelected?'var(--primary)':'var(--muted)'}}>{isSelected?'☑':'☐'}</span>}{q}{!selectMode&&<button onClick={()=>removeQuery(i)} style={{background:'none',border:'none',color:'var(--muted)',cursor:'pointer',fontSize:14,padding:0,lineHeight:1}}>×</button>}</span>;})}{queries.length===0&&<span style={{fontSize:12,color:'var(--muted)'}}>No queries yet. Add some below.</span>}</div>
