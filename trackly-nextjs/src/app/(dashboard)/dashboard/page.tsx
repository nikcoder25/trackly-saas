...
export default function DashboardPage() {
  ...
  const addQuery = () => {
    ...
    const addedQuery = newQuery.trim();
    const updated = [...queries, addedQuery];
    fetch(`/api/brands/${brand.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify({ queries: updated }) })
      .then(async (res) => {
        if (!res.ok) {
          ...
          setQueryMsg(err.error || 'Failed to save query');
          return;
        }
        setNewQuery(''); fetchBrands(); setTimeout(() => startRunRef.current(false, { auto: true, queries: [addedQuery] }), 600);
      });
  };
  ...
  if (!brand) return (
    <div className="bg-[var(--bg2)] border border-[var(--border)] rounded-xl p-8 text-center shadow-[var(--app-shadow)]">
      ...
      <Link href="/dashboard/setup" className="inline-block bg-[var(--primary)] hover:bg-[var(--primary-hover)] text-white px-6 py-2.5 rounded-lg font-bold text-sm transition no-underline shadow-[0_1px_2px_rgba(99,102,241,.2)]">Set Up Brand</Link>
    </div>
  );
  ...
  return (
    <div>
      ...
      {show('queries')&&<div className="ov-card" style={{marginBottom:0}}><div className="ov-card-head"><div className="ov-card-title">Tracked Queries</div><div className="ov-card-sub">{queries.length} / {planLimit>1000?'∞':planLimit} prompts</div></div>
        <div className="ov-query-tags" style={{display:'flex',flexWrap:'wrap',gap:6,marginBottom:12}}>{queries.map((q,i)=>{const isSelected=selectedQueries.has(i);const toggle=()=>{setSelectedQueries(prev=>{const next=new Set(prev);if(next.has(i))next.delete(i);else next.add(i);return next;});};return <span key={i} title={selectMode?(isSelected?'Click to unselect':'Click to select'):q} role={selectMode?'button':undefined} aria-pressed={selectMode?isSelected:undefined} tabIndex={selectMode?0:undefined} onClick={selectMode?toggle:undefined} onKeyDown={selectMode?(e=>{if(e.key===' '||e.key==='Enter'){e.preventDefault();toggle();}}):undefined} style={{display:'inline-flex',alignItems:'center',gap:6,background:selectMode&&isSelected?'var(--primary)':'var(--bg3)',border:selectMode?(isSelected?'1px solid var(--primary)':'1px dashed var(--primary-border, rgba(99,102,241,.4))'):'1px solid var(--border)',color:selectMode&&isSelected?'#fff':'var(--text)',fontSize:12,fontWeight:selectMode&&isSelected?600:400,padding:'6px 12px',borderRadius:100,cursor:selectMode?'pointer':'default',userSelect:'none',transition:'background .15s,border-color .15s,color .15s',boxShadow:selectMode&&isSelected?'0 0 0 2px rgba(99,102,241,.25)':'none'}}>{selectMode&&<span aria-hidden="true" style={{fontSize:14,lineHeight:1,color:isSelected?'#fff':'var(--primary)'}}>{isSelected?'☑':'☐'}</span>}{q}{!selectMode&&<button onClick={()=>removeQuery(i)} style={{background:'none',border:'none',color:'var(--muted)',cursor:'pointer',fontSize:14,padding:0,lineHeight:1}}>×</button>}</span>;})}{queries.length===0&&<span style={{fontSize:12,color:'var(--muted)'}}>No queries yet. Add some below.</span>}</div>
        {selectMode&&<div style={{display:'flex',alignItems:'center',gap:8,marginBottom:12,padding:'8px 12px',background:'var(--primary-light, rgba(99,102,241,.08))',border:'1px solid var(--primary-border, rgba(99,102,241,.25))',borderRadius:'var(--radius-xs)',fontSize:12,flexWrap:'wrap'}}>
          <span style={{color:'var(--primary)',fontWeight:600}}>{selectedQueries.size} of {queries.length} selected</span>
          <button className="ov-btn-subtle" disabled={!queries.length} onClick={()=>{if(selectedQueries.size===queries.length)setSelectedQueries(new Set());else setSelectedQueries(new Set(queries.map((_,i)=>i)));}}>{selectedQueries.size===queries.length&&queries.length>0?'UNSELECT ALL':'SELECT ALL'}</button>
          <button className="ov-btn-subtle" disabled={selectedQueries.size===0} onClick={()=>{const picked=queries.filter((_,i)=>selectedQueries.has(i));navigator.clipboard.writeText(picked.join('\n')).then(()=>setQueryMsg(`Copied ${picked.length} ${picked.length===1?'query':'queries'} to clipboard`)).catch(()=>setQueryMsg('Failed to copy queries'));}}>📋 COPY SELECTED</button>
          <button className="ov-btn-subtle ov-btn-red" disabled={selectedQueries.size===0||!brand} onClick={()=>{if(!brand)return;const updated=queries.filter((_,i)=>!selectedQueries.has(i));const removedCount=queries.length-updated.length;fetch(`/api/brands/${brand.id}`,{method:'PUT',headers:{'Content-Type':'application/json'},credentials:'include',body:JSON.stringify({queries:updated})}).then(async(res)=>{if(!res.ok){const err=await res.json().catch(()=>({}));setQueryMsg(err.error||'Failed to delete queries');return;}fetchBrands();setSelectedQueries(new Set());setSelectMode(false);setQueryMsg(`Removed ${removedCount} ${removedCount===1?'query':'queries'}`);});}}>🗑 DELETE SELECTED</button>
        </div>}
        ...
        <div className="ov-query-btns" style={{display:'flex',gap:6,flexWrap:'wrap'}}>
          ...
          <button className="ov-btn-subtle" disabled={!queries.length} onClick={()=>{navigator.clipboard.writeText(queries.join('\n')).then(()=>{setQueryMsg('Copied all queries to clipboard');}).catch(()=>{setQueryMsg('Failed to copy queries');});}}>📋 COPY ALL</button>
        </div>
        {queryMsg && <div style={{marginTop:8,padding:'8px 14px',background:queryMsg.includes('Added')||queryMsg.includes('Copied')?'rgba(34,197,94,.08)':'rgba(245,158,11,.08)',border:queryMsg.includes('Added')||queryMsg.includes('Copied')?'1px solid rgba(34,197,94,.2)':'1px solid rgba(245,158,11,.2)',borderRadius:'var(--radius-xs)',fontSize:12,color:queryMsg.includes('Added')||queryMsg.includes('Copied')?'var(--green)':'var(--amber)'}}>{queryMsg}</div>}
        {bulkAddOpen&&<div style={{marginTop:12,background:'var(--bg)',border:'1px solid var(--border)',borderRadius:'var(--radius-xs)',padding:14}}><textarea value={bulkText} onChange={e=>setBulkText(e.target.value)} rows={5} style={{width:'100%',background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:'var(--radius-xs)',padding:10,fontSize:13,fontFamily:'var(--font)',color:'var(--text)',resize:'vertical'}} placeholder={'Paste queries (one per line)'}/><div style={{display:'flex',gap:8,marginTop:8}}><button className="pbtn" style={{background:'var(--primary)',color:'#fff',borderColor:'var(--primary)'}} onClick={()=>{const lines=bulkText.split('\n').map(l=>l.trim()).filter(l=>l.length>0);if(lines.length&&brand){const updated=[...queries,...lines];fetch(`/api/brands/${brand.id}`,{method:'PUT',headers:{'Content-Type':'application/json'},credentials:'include',body:JSON.stringify({queries:updated})}).then(async(res)=>{if(!res.ok){const err=await res.json().catch(()=>({}));setQueryMsg(err.error||'Failed to save queries');return;}fetchBrands();setBulkText('');setBulkAddOpen(false);setTimeout(()=>startRunRef.current(false,{auto:true,queries:lines}),600);});}}}>Add {bulkText.split('\n').filter(l=>l.trim()).length} Queries</button><button className="pbtn" onClick={()=>setBulkAddOpen(false)}>Cancel</button></div></div>}
