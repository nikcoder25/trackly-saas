    ...
    <div>
      ...
      {show('queries')&&<div className="ov-card" style={{marginBottom:0}}><div className="ov-card-head"><div className="ov-card-title">Tracked Queries</div><div className="ov-card-sub">{queries.length} / {planLimit>1000?'∞':planLimit} prompts</div></div>
        ...
        <div className="ov-query-btns" style={{display:'flex',gap:6,flexWrap:'wrap'}}>
          ...
          <button className="ov-btn-subtle ov-btn-green" onClick={()=>{if(!brand)return;const industry=brand.industry||'services';const city=brand.city||'';const suggestions=[`best ${industry} in ${city||'my area'}`,`top rated ${industry} near me`,`${brand.name} reviews`,`recommended ${industry} companies`,`${industry} cost ${city}`].filter(s=>!queries.includes(s));if(suggestions.length){const updated=[...queries,...suggestions];fetch(`/api/brands/${brand.id}`,{method:'PUT',headers:{'Content-Type':'application/json'},credentials:'include',body:JSON.stringify({queries:updated})}).then(()=>fetchBrands());}}}>SUGGEST</button>
          <button className="ov-btn-subtle ov-btn-blue" onClick={()=>{if(!brand)return;const generated=[`what is ${brand.name}`,`is ${brand.name} good`,`${brand.name} vs competitors`,`${brand.name} pricing`,`alternatives to ${brand.name}`].filter(s=>!queries.includes(s));if(generated.length){const updated=[...queries,...generated];fetch(`/api/brands/${brand.id}`,{method:'PUT',headers:{'Content-Type':'application/json'},credentials:'include',body:JSON.stringify({queries:updated})}).then(()=>fetchBrands());}}}>AI GENERATE</button>
