  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setShowCreate(true)} className="bg-[var(--primary)] hover:bg-[var(--primary-hover)] text-white px-4 py-2 rounded-lg text-sm font-medium transition">+ New Brand</button>
      </div>
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
          {brands.map(b => (
            <button key={b.id} onClick={() => { setSelectedBrand(b); setShowCreate(false); }}
              className={`shrink-0 px-4 py-2 rounded-lg text-sm transition ${selectedBrand?.id === b.id && !showCreate ? 'bg-[var(--primary)] text-white' : 'bg-[var(--bg2)] text-[var(--text-muted)] hover:bg-[var(--bg3)] border border-[var(--border)]'}`}>{b.name}</button>
          ))}
        </div>
      )}
...
function CreateBrandForm({ onCreated }: { onCreated: (brand: Brand) => void }) {
  ...
  return (
    <div className="bg-[var(--bg2)] border border-[var(--border)] rounded-xl p-6">
      <h2 className="text-lg font-semibold text-white mb-4">Create New Brand</h2>
      {error && <div className="bg-red-900/20 border border-red-800 text-red-400 text-sm px-4 py-3 rounded-lg mb-4">{error}</div>}
      <form onSubmit={handleCreate} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div><label className="block text-sm text-[var(--text-muted)] mb-1">Brand Name *</label>
            <input value={name} onChange={e => setName(e.target.value)} required className="w-full bg-[var(--bg)] border border-[var(--border)] rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:border-[var(--primary)]" placeholder="e.g. Acme Corp" /></div>
          <div><label className="block text-sm text-[var(--text-muted)] mb-1">Industry</label>
            <input value={industry} onChange={e => setIndustry(e.target.value)} className="w-full bg-[var(--bg)] border border-[var(--border)] rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:border-[var(--primary)]" placeholder="e.g. CRM Software" /></div>
          <div><label className="block text-sm text-[var(--text-muted)] mb-1">Website</label>
            <input value={website} onChange={e => setWebsite(e.target.value)} className="w-full bg-[var(--bg)] border border-[var(--border)] rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:border-[var(--primary)]" placeholder="https://acme.com" /></div>
...
function EditBrandForm({ brand, onUpdated, onDeleted }: { brand: Brand; onUpdated: (b: Brand) => void; onDeleted: () => void }) {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-[var(--bg2)] border border-[var(--border)] rounded-xl p-6">
        <h2 className="text-lg font-semibold text-white mb-4">Edit Brand</h2>
        {error && <div className="bg-red-900/20 border border-red-800 text-red-400 text-sm px-4 py-3 rounded-lg mb-4">{error}</div>}
        {message && <div className="bg-green-900/20 border border-green-800 text-green-400 text-sm px-4 py-3 rounded-lg mb-4">{message}</div>}
        <form onSubmit={handleSave} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label className="block text-sm text-[var(--text-muted)] mb-1">Brand Name</label>
              <input value={name} onChange={e => setName(e.target.value)} required className="w-full bg-[var(--bg)] border border-[var(--border)] rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:border-[var(--primary)]" /></div>
            <div><label className="block text-sm text-[var(--text-muted)] mb-1">Industry</label>
              <input value={industry} onChange={e => setIndustry(e.target.value)} className="w-full bg-[var(--bg)] border border-[var(--border)] rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:border-[var(--primary)]" /></div>
            <div><label className="block text-sm text-[var(--text-muted)] mb-1">Website</label>
              <input value={website} onChange={e => setWebsite(e.target.value)} className="w-full bg-[var(--bg)] border border-[var(--border)] rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:border-[var(--primary)]" /></div>
            <div><label className="block text-sm text-[var(--text-muted)] mb-1">City</label>
              <input value={city} onChange={e => setCity(e.target.value)} className="w-full bg-[var(--bg)] border border-[var(--border)] rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:border-[var(--primary)]" /></div>
          </div>
          <div>
            ...
            <textarea value={queries} onChange={e => setQueries(e.target.value)} rows={6}
              className="w-full bg-[var(--bg)] border border-[var(--border)] rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:border-[var(--primary)] font-mono"
      ...
      <div className="bg-[var(--bg2)] border border-[var(--border)] rounded-xl p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div><p className="text-[var(--text-muted)]">Queries</p><p className="text-white font-semibold">{brand.queries?.length || 0}</p></div>
          <div><p className="text-[var(--text-muted)]">Competitors</p><p className="text-white font-semibold">{brand.competitors?.length || 0}</p></div>
          <div><p className="text-[var(--text-muted)]">Runs</p><p className="text-white font-semibold">{Array.isArray(brand.runs) ? brand.runs.length : 0}</p></div>
