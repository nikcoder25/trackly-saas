  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="text-2xl font-bold text-white">Alerts</h1><p className="text-[var(--text-muted)] mt-1">Set up alert rules and notifications</p></div>
        <button onClick={() => setShowCreate(!showCreate)} className="bg-[var(--primary)] hover:bg-[var(--primary-hover)] text-white px-4 py-2 rounded-lg text-sm font-medium transition">+ New Alert</button>
      </div>
        ...
        <div className="bg-[var(--bg2)] border border-[var(--border)] rounded-xl p-5 mb-4">
          ...
          <div className="flex gap-3 items-end">
            <div className="flex-1"><label className="block text-xs text-[var(--text-muted)] mb-1">Alert Name</label>
              <input value={newName} onChange={e => setNewName(e.target.value)} className="w-full bg-[var(--bg)] border border-[var(--border)] rounded-lg px-3 py-2 text-white text-sm" placeholder="e.g. SOV Drop Alert" /></div>
            <div><label className="block text-xs text-[var(--text-muted)] mb-1">Type</label>
              <select value={newType} onChange={e => setNewType(e.target.value)} className="bg-[var(--bg)] border border-[var(--border)] rounded-lg px-3 py-2 text-white text-sm">
            ...
            <button onClick={handleCreate} disabled={saving} className="bg-[var(--primary)] text-white px-4 py-2 rounded-lg text-sm disabled:opacity-50">{saving ? '...' : 'Create'}</button>
          </div>
        </div>
      )}
        ...
        <div className="space-y-2">
          {alerts.map(a => (
            <div key={a.id} className="bg-[var(--bg2)] border border-[var(--border)] rounded-lg p-4 flex items-center gap-4">
              ...
              <div className="flex-1"><p className="text-sm text-white font-medium">{a.name}</p>
              ...
              <button onClick={() => deleteAlert(a.id)} className="text-[var(--text-muted)] hover:text-red-400 text-xs">Delete</button>
            </div>
          ))}
        </div>
      )}
