...
// Self-Serve Connect (M1) — the "Connect your website" screen.
// One paste, done: create a snippet connection, show the one-liner + copy
...
export function PageConnect() {
  ...
  // Create (or fetch) the brand's snippet connection once we know the brand.
  const start = React.useCallback(async (id: string) => {
    ...
    try {
      const d = await api(`/api/brands/${id}/connections`, {
        ...
        body: JSON.stringify({ method: 'snippet' }),
      });
  ...
  React.useEffect(() => { if (brandId) start(brandId); }, [brandId, start]);
  ...
  return (
    <>
      <PageHead title="Connect your website" sub="One paste, done — no DNS, no ownership challenge, no waiting." />
      <div className="page-body" style={{ display: 'grid', gap: 16, maxWidth: 720 }}>
        <Card title="Paste a snippet" right={conn ? chip : undefined} lede="The easy default — works on any custom-coded site. Client-side.">
          ...
          {snippet && (
            <>
              <p className="quiet" style={{ margin: '0 0 10px', fontSize: 13, lineHeight: 1.6 }}>
                Paste this one line into your site&rsquo;s <code>&lt;head&gt;</code> (or just before <code>&lt;/body&gt;</code>).
                It applies your shipped SEO fixes to each page and reports back here — no other setup.
              </p>
              <div
                style={{
