...
// Self-service plan change with payment stub
router.post('/upgrade', auth, async (req, res) => {
  ...
  if (tiers[plan] > tiers[currentPlan]) {
    // Return checkout info stub — integrate with Stripe/LemonSqueezy here
    const prices = { pro: 'price_pro_monthly_2900', agency: 'price_agency_monthly_7900' };
    return res.json({
      requiresPayment: true,
      plan,
      checkoutUrl: process.env.STRIPE_CHECKOUT_URL || null,
      priceId: prices[plan] || null,
      message: `To upgrade to ${plan}, complete payment via the checkout flow.`
    });
  }
  try {
