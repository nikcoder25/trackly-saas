  ...
  try {
    ...
    const result = await pool.query(`
      SELECT u.*, COUNT(DISTINCT b.id)::int AS brand_count,
             COALESCE(SUM(jsonb_array_length(COALESCE(b.data->'queries', '[]'::jsonb))), 0)::int AS tracked_queries
      FROM users u
    ...
    const users = result.rows.map(row => ({ ...safeUser(row), brandCount: row.brand_count || 0, trackedQueries: row.tracked_queries || 0 }));
...
router.put('/admin/users/:id', auth, requireAdmin, async (req, res) => {
  try {
    ...
    if (plan !== undefined) {
      if (!['free', 'starter', 'pro', 'agency', 'enterprise', 'owner'].includes(plan)) return res.status(400).json({ error: 'Invalid plan' });
...
router.post('/admin/users', auth, requireAdmin, async (req, res) => {
  try {
    ...
    if (plan && !['free', 'starter', 'pro', 'agency', 'enterprise', 'owner'].includes(plan)) return res.status(400).json({ error: 'Invalid plan' });
...
router.put('/admin/users/:id/plan', auth, requireAdmin, async (req, res) => {
  try {
    if (!['free', 'starter', 'pro', 'agency', 'enterprise', 'owner'].includes(req.body.plan)) return res.status(400).json({ error: 'Invalid plan' });
